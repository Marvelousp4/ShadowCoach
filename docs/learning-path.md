...
The local extractor returns high-confidence language patterns, including productive relationships such as `compare A with B`, `either A or B`, and `move from A to B`. It does not fall back to arbitrary n-grams, names, IDs, numbers, or topic-specific noun phrases. Local Codex automatically checks and improves candidates when enabled; the secondary button simply reruns that selection. Gemini remains an explicit, provider-labelled action so the app does not create an unrequested paid call. AI candidates must still pass local validation and remain exact substrings of the source sentence. Valid AI targets are merged with strong local candidates, so an empty or malformed model response cannot erase a useful deterministic result. Versioned target-selection caches cause older empty results to be inspected again after the selection policy improves.
