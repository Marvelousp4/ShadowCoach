...
The deeper manual release audit also installs local analysis into a temporary HOME, generates an original TTS fixture, runs WhisperX with `tiny.en`, checks word timestamps, and runs `prosody_analyzer.py`. This deeper test downloads approximately 1–2 GB and is intentionally not part of every CI run.
