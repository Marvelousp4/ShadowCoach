...
1. 结合语境理解真实输入。
2. 只保留真正可迁移的句式、固定表达、搭配或短语动词；没有高价值目标时就练整句。
3. Shadowing 模仿原声节奏和表达。
4. 隐藏答案，无提示大声提取。
5. 用本地 FSRS-6 安排间隔复习。
6. 保留学习目标，但真正换人物、时间和场景重新表达。
7. 围绕相关经历或观点自由表达 30–60 秒。
8. 在真实对话、会议或语音消息中使用一次，并记录实际效果。
9. 分析一次精确复述，根据反馈完成一遍纠错重录。

开放表达不会被当作“没有照抄原句”而扣分。换情境造句检查迁移是否成立，自由表达检查沟通和自然度，真实使用只根据用户记录的实际原话与结果反馈。各步骤完成条件见 [学习路径说明](docs/learning-path.md)。
